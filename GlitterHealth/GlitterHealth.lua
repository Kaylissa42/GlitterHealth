...
local function ensureDayShape(d) d.steps=d.steps or 0; d.calories=d.calories or 0; d.exerciseSec=d.exerciseSec or 0; d.distanceFootYd=d.distanceFootYd or 0; d.distanceSwimYd=d.distanceSwimYd or 0; d.jumps=d.jumps or 0 end
...
local function refreshLiveStats()
  ...
  if live.milesWalk then live.milesWalk:SetText(string.format("Miles (Walk): %.2f", yd_to_mi(d.distanceFootYd or 0))) end
  if live.jumps     then live.jumps:SetText(string.format("Jumps: %d", safeint(d.jumps or 0))) end
  if live.milesSwim then live.milesSwim:SetText(string.format("Miles (Swim): %.2f", yd_to_mi(d.distanceSwimYd or 0))) end
  if live.exMin     then live.exMin:SetText(string.format("Exercise (Min): %d", safeint((d.exerciseSec or 0)/60))) end
  ...
  if live.lt_steps or live.lt_foot or live.lt_jumps or live.lt_swim or live.lt_ex or live.lt_kcal then
    ...
    if live.lt_steps then live.lt_steps:SetText(string.format("Steps (Total): %d", T.steps)) end
    if live.lt_foot  then live.lt_foot:SetText(string.format("Miles Walked: %.2f", yd_to_mi(T.footYd))) end
    if live.lt_jumps then live.lt_jumps:SetText(string.format("Jumps (Total): %d", T.jumps)) end
    if live.lt_swim  then live.lt_swim:SetText(string.format("Miles Swum: %.2f", yd_to_mi(T.swimYd))) end
    if live.lt_ex    then live.lt_ex:SetText(string.format("Exercise (Total Min): %d", T.exMin)) end
    if live.lt_kcal  then live.lt_kcal:SetText(string.format("Calories (Total): %d", T.kcal)) end
  end
...
local function openCSVWindow()
  if not NS.well.csv then
    ...
    f:SetBackdrop({ bgFile="Interface\\DialogFrame\\UI-DialogBox-Background-Dark", edgeFile="Interface\\DialogFrame\\UI-DialogBox-Border", edgeSize=16, insets={left=4,right=4,top=4,bottom=4} })
    f:SetBackdropColor(0,0,0,0.92)
    f:EnableMouse(true); f:SetMovable(true)
    f:RegisterForDrag("LeftButton")
    f:SetScript("OnDragStart", function(s) s:StartMoving() end)
    f:SetScript("OnDragStop", function(s) s:StopMovingOrSizing() end)
    local title=f:CreateFontString(nil,"OVERLAY","GameFontHighlightLarge"); title:SetPoint("TOPLEFT",12,-10); title:SetText("|cFFFF77FFGlitterHealth|r — CSV Export")
    ...
    editBox:SetPoint("TOPLEFT", 20, -40); editBox:SetPoint("BOTTOMRIGHT", -20, 60)
    ...
    inset:SetPoint("TOPLEFT", 12, -34); inset:SetPoint("BOTTOMRIGHT", editBox, "BOTTOMRIGHT", 8, -8)
    local sel = CreateFrame("Button", nil, f, "UIPanelButtonTemplate"); sel:SetSize(120, 22); sel:SetPoint("BOTTOMLEFT", 14, 18); sel:SetText("Select All")
    sel:SetScript("OnClick", function() editBox:HighlightText() editBox:SetFocus() end)
    local closeBtn = CreateFrame("Button", nil, f, "UIPanelButtonTemplate"); closeBtn:SetSize(100, 22); closeBtn:SetPoint("BOTTOMRIGHT", -14, 18); closeBtn:SetText("Close"); closeBtn:SetScript("OnClick", function() f:Hide() end)
    f.editBox = editBox
    NS.well.csv = f
  end
  ...
  f.editBox:SetText(buildCSV()); f:Show()
end
...
function NS.ToggleWellbeing()
  ...
  H("Vibes")
  local S = DB.stats or {}
  live.hugs    = Cell("Total Hugs",   safeint(S.hugs),   0)
  live.cheers  = Cell("Total Cheers", safeint(S.cheers), 260); y = y - 18
  live.lols    = Cell("Total LOLs",   safeint(S.lols),   0)
  live.waves   = Cell("Total Waves",  safeint(S.waves),  260); y = y - 18
  live.kisses  = Cell("Total Kisses", safeint(S.kisses), 0)
  live.dances  = Cell("Total Dances", safeint(S.dances),260); y = y - 18
  live.pats    = Cell("Total Pats",   safeint(S.pats),   0)
  live.claps   = Cell("Total Claps",  safeint(S.claps),  260); y = y - 18
  live.boops   = Cell("Total Boops",  safeint(S.boops),  0);   y = y - 12

  -- Vibe score (smaller & purple)
  y = y - 6
  live.vibeFS = c:CreateFontString(nil,"OVERLAY","GameFontHighlight")
  do local fnt, size, flags = live.vibeFS:GetFont(); live.vibeFS:SetFont(fnt, math.max(10, size-2), flags) end
  live.vibeFS:SetPoint("TOPLEFT", 0, y)
  live.vibeFS:SetWidth(480); live.vibeFS:SetJustifyH("LEFT")
  live.vibeFS:SetText(colorize(VIBE_HEX, "Vibe Score: 0%"))
  y = y - (live.vibeFS:GetStringHeight() + 14)

  -- Today's Wins
  L(" ")
  H("Today's Wins")
  local d = (DB.data and DB.data[DB.today]) or {}
  live.steps     = Cell("Steps",          safeint(d.steps or 0),                                0)
  live.milesWalk = Cell("Miles (Walk)",   string.format("%.2f", yd_to_mi(d.distanceFootYd or 0)), 260); y = y - 18
  live.jumps     = Cell("Jumps",          safeint(d.jumps or 0),                                0)
  live.milesSwim = Cell("Miles (Swim)",   string.format("%.2f", yd_to_mi(d.distanceSwimYd or 0)), 260); y = y - 18
  live.exMin     = Cell("Exercise (Min)", safeint((d.exerciseSec or 0)/60),                     0)
  live.kcal      = Cell("Calories",       safeint(d.calories or 0),                             260); y = y - 8

  -- Lifetime Wins
  L(" ")
  H("Lifetime Wins")
  live.lt_steps = Cell("Steps (Total)",     0,   0)
  live.lt_foot  = Cell("Miles Walked",      "0.00", 260); y = y - 18
  live.lt_jumps = Cell("Jumps (Total)",     0,   0)
  live.lt_swim  = Cell("Miles Swum",        "0.00", 260); y = y - 18
  live.lt_ex    = Cell("Exercise (Total Min)", 0, 0)
  live.lt_kcal  = Cell("Calories (Total)",  0,   260); y = y - 8
...
local function goalNotify(key, label)
  ...
  if DB.runtime.goalNotified[key] then return end
  ...
  if (DB.profile.enableGoalSound ~= false) and not DB.runtime.levelSoundPlayedT[key] then
    if SOUNDKIT and SOUNDKIT.UI_GUILD_LEVEL_UP then
      PlaySound(SOUNDKIT.UI_GUILD_LEVEL_UP)
    else
      PlaySound(888)
    end
    DB.runtime.levelSoundPlayedT[key] = true
  end
  if (DB.profile.enableCelebration ~= false) and NS and NS.ShowCelebration then NS.ShowCelebration(label) end
end
...
local function updateBars()
  ...
  local mv = clamp((d.calories or 0)/math.max(g.calories,1), 0, 1)
  local st = clamp((d.steps or 0)/math.max(g.steps,1), 0, 1)
  local ex = clamp(((d.exerciseSec or 0)/60)/math.max(g.exerciseMin,1), 0, 1)
  local miW = yd_to_mi(d.distanceFootYd or 0)
  local miS = yd_to_mi(d.distanceSwimYd or 0)
  local jp  = (d.jumps or 0)

  if DB.profile.show.move then ui.moveBar.bar:SetValue(mv); ui.moveBar.text:SetText(string.format("Move: %d / %d Kcal", safeint(d.calories), g.calories)) end
  if DB.profile.show.steps then ui.stepBar.bar:SetValue(st); ui.stepBar.text:SetText(string.format("Steps: %d / %d", safeint(d.steps), g.steps)) end
  if DB.profile.show.exer then ui.exerBar.bar:SetValue(ex); ui.exerBar.text:SetText(string.format("Exercise: %d / %d Min", safeint((d.exerciseSec or 0)/60), g.exerciseMin)) end
  if DB.profile.show.walk then ui.walkBar.bar:SetValue(clamp(miW/math.max(g.milesWalk,0.01), 0, 1)); ui.walkBar.text:SetText(string.format("Walk: %0.2f / %0.2f Mi", miW, g.milesWalk)) end
  if DB.profile.show.swim then ui.swimBar.bar:SetValue(clamp(miS/math.max(g.milesSwim,0.01), 0, 1)); ui.swimBar.text:SetText(string.format("Swim: %0.2f / %0.2f Mi", miS, g.milesSwim)) end
  if DB.profile.show.jump then ui.jumpBar.bar:SetValue(clamp(jp/math.max(g.jumps,1), 0, 1)); ui.jumpBar.text:SetText(string.format("Jumps: %d / %d", safeint(jp), g.jumps)) end

  if mv >= 1 then goalNotify("move", "Move") end
  if st >= 1 then goalNotify("steps", "Steps") end
  if ex >= 1 then goalNotify("exer", "Exercise") end
  if miW >= g.milesWalk and g.milesWalk>0 then goalNotify("walk", "Walk") end
  if miS >= g.milesSwim and g.milesSwim>0 then goalNotify("swim", "Swim") end
  if jp >= g.jumps and g.jumps>0 then goalNotify("jump", "Jumps") end
...
NS.ShowCelebration = NS.ShowCelebration or function(goalLabel)
  local scrW  = (UIParent and UIParent:GetWidth())  or 1920
  local scrH  = (UIParent and UIParent:GetHeight()) or 1080
  if not goalLabel then goalLabel = "Goal Complete!" end

  if not NS.celebration then NS.celebration = {} end
  local CFX = NS.celebration
  if not CFX.frame then
    local f = CreateFrame("Frame", "GlitterHealthCelebration", UIParent, "BackdropTemplate")
    f:SetFrameStrata("FULLSCREEN_DIALOG")
    f:SetAllPoints(UIParent)
    f:Hide()

    local msg = f:CreateFontString(nil, "OVERLAY", "GameFontNormalHuge")
    msg:SetPoint("TOP", UIParent, "TOP", 0, -math.floor(scrH * 0.18))
    do local fn, sz, fl = msg:GetFont(); msg:SetFont(fn, (sz or 32) * 1.8, "OUTLINE") end
    msg:SetTextColor(1, 1, 1, 1)
    f.msg = msg

    -- rainbow bar anchor
    local rb = f:CreateTexture(nil, "OVERLAY")
    rb:SetPoint("TOP", UIParent, "TOP", 0, -math.floor(scrH * 0.25))
    rb:SetSize(scrW * 1.10, 48)
    rb:SetTexture("Interface\\Buttons\\WHITE8X8")
    rb:SetVertexColor(1,1,1,1)
    ...
    for i=1,#colors do
      ...
      stripe:SetVertexColor(colors[i][1],colors[i][2],colors[i][3], 0.95)
      stripe:SetPoint("CENTER", rb, "CENTER", 0, (i-4)*12)
      stripe:SetSize(scrW * 1.12, 16)
      CFX.rainbow[#CFX.rainbow+1] = stripe
    end

    local conf = CreateFrame("Frame", nil, f)
    conf:SetAllPoints(f)
    conf:SetFrameLevel((f:GetFrameLevel() or 5) + 10)
    CFX.confetti = conf
    CFX.confettiTex = {}

    local ag = f:CreateAnimationGroup()
    local fadeIn  = ag:CreateAnimation("Alpha"); fadeIn:SetFromAlpha(0); fadeIn:SetToAlpha(1); fadeIn:SetDuration(0.25)
    local hold    = ag:CreateAnimation("Alpha"); hold:SetFromAlpha(1); hold:SetToAlpha(1); hold:SetDuration(3.00)
    local fadeOut = ag:CreateAnimation("Alpha"); fadeOut:SetFromAlpha(1); fadeOut:SetToAlpha(0); fadeOut:SetDuration(1.50); fadeOut:SetOrder(3)
    ag:SetScript("OnFinished", function() f:Hide() end)
    CFX.anim = ag

    CFX.frame = f; CFX.msg = msg
  end

  local f    = NS.celebration.frame
  local conf = NS.celebration.confetti
  local msg  = NS.celebration.msg
  local anim = NS.celebration.anim

  msg:SetText(("|cFFFFEE88CONGRATULATIONS!|r  %s Goal Reached!"):format(goalLabel))

  -- clear old pieces
  for _,t in ipairs(NS.celebration.confettiTex or {}) do t:Hide() end
  NS.celebration.confettiTex = {}

  local colors2 = {
    {1,0.3,0.3},{1,0.6,0.2},{1,0.9,0.2},
    {0.3,0.9,0.3},{0.3,0.6,1},{0.6,0.4,1},{1,0.3,1},{1,1,1}
  }

  local pieces = 240
  for i=1,pieces do
    local t = conf:CreateTexture(nil, "OVERLAY")
    t:SetTexture("Interface\\Buttons\\WHITE8X8")
    local c = colors2[((i-1) % #colors2) + 1]
    t:SetVertexColor(c[1], c[2], c[3], 1)
    local size = math.random(6, 12)
    t:SetSize(size, size)
    t:SetPoint("TOPLEFT",
      math.random(0, math.floor(scrW)),
      -math.random(math.floor(scrH*0.15), math.floor(scrH*0.35))
    )

    local ag = t:CreateAnimationGroup()
    local trans = ag:CreateAnimation("Translation")
    trans:SetOffset(math.random(-120,120), -math.random(400, 700))
    trans:SetDuration(math.random(900,1300)/1000)

    local rot = ag:CreateAnimation("Rotation")
    rot:SetDegrees(math.random(90,360))
    rot:SetDuration(trans:GetDuration())

    local fade = ag:CreateAnimation("Alpha")
    fade:SetFromAlpha(1); fade:SetToAlpha(0)
    fade:SetDuration(trans:GetDuration())
    fade:SetOrder(2)

    ag:SetScript("OnFinished", function() t:Hide() end)
    ag:Play()

    table.insert(NS.celebration.confettiTex, t)
  end

  f:Show(); anim:Stop(); anim:Play()
end
...
-- Calories (Move) Goal
slider(secC,"left","Move Goal (Kcal)", 100, 3000, 10,
  function() return math.floor((DB.profile.goals.calories or 500)+0.5) end,
  function(v) DB.profile.goals.calories = clamp(math.floor(v+0.5), 100, 3000) end,
  "%d","%d")

-- Steps Goal
slider(secC,"right","Steps Goal", 1000, 50000, 500,
  function() return math.floor((DB.profile.goals.steps or 10000)+0.5) end,
  function(v) DB.profile.goals.steps = clamp(math.floor(v+0.5), 1000, 50000) end,
  "%d","%d")

-- Exercise Minutes Goal
slider(secC,"left","Exercise Goal (Min)", 5, 240, 5,
  function() return math.floor((DB.profile.goals.exerciseMin or 30)+0.5) end,
  function(v) DB.profile.goals.exerciseMin = clamp(math.floor(v+0.5), 5, 240) end,
  "%d","%d")

-- Walk Miles Goal
slider(secC,"right","Walk Miles Goal", 0.0, 20.0, 0.1,
  function() return round(DB.profile.goals.milesWalk or 5.0, 2) end,
  function(v) DB.profile.goals.milesWalk = clamp(round(v,2), 0.0, 20.0) end,
  "%.2f","%.2f")

-- Swim Miles Goal
slider(secC,"left","Swim Miles Goal", 0.0, 10.0, 0.1,
  function() return round(DB.profile.goals.milesSwim or 1.0, 2) end,
  function(v) DB.profile.goals.milesSwim = clamp(round(v,2), 0.0, 10.0) end,
  "%.2f","%.2f")

-- Jumps Goal
slider(secC,"right","Jumps Goal", 100, 10000, 100,
  function() return math.floor((DB.profile.goals.jumps or 1000)+0.5) end,
  function(v) DB.profile.goals.jumps = clamp(math.floor(v+0.5), 100, 10000) end,
  "%d","%d")
...
check (secN,"right","Show Celebration FX","Shows big congratulations + confetti + rainbow when a goal is reached.",
...
button(secN,"right","Test Celebration",   function() NS.TestCelebration() end)
sectionFinish(secN)
...
local function onEmoteMessage(event, msg, sender)
  ...
  local involvesMe = (sender == me) or (me and mlow:find(me:lower(), 1, true) ~= nil)
...
core:SetScript("OnEvent", function(self, event, arg1, arg2)
  ...
  elseif event=="PLAYER_LOGIN" then
    ...
    C_Timer.After(0.2, function()
      if ui.frame then local nx,ny = clampToScreen(ui.frame); DB.profile.frame.x, DB.profile.frame.y = nx, ny end
      print("|cFFFF77FFGlitterHealth loaded!|r")
    end)
    self:SetScript("OnUpdate", function(_, elapsed)
      local tk=todayKey()
      if tk ~= DB.today then DB.today=tk; DB.runtime.carryStepDist=0; DB.runtime.goalNotified = {}; DB.runtime.levelSoundPlayedT = {} end
      pcall(function() tick(elapsed) end)
    end)
...
-- ========= slash =========
SLASH_GLITTERHEALTH1="/glitterhealth"; SLASH_GLITTERHEALTH2="/gh"
SlashCmdList["GLITTERHEALTH"]=function(msg)
  msg=(msg or ""):lower()
  if msg=="lock" then DB.profile.frame.locked=true; print("|cFFFF77FFGlitterHealth|r: Frame Locked.")
  elseif msg=="unlock" then DB.profile.frame.locked=false; print("|cFFFF77FFGlitterHealth|r: Frame Unlocked. Drag Me!")
  elseif msg=="reset" then DB.data[DB.today]=nil; DB.runtime.carryStepDist=0; updateBars(); print("|cFFFF77FFGlitterHealth|r: Today Reset.")
  elseif msg=="mood" or msg=="well" or msg=="wellbeing" then NS.ToggleWellbeing()
  elseif msg=="options" or msg=="opt" or msg=="settings" then SettingsUtil:Open()
  elseif msg=="hide" then setHidden(true); print("|cFFFF77FFGlitterHealth|r: Bars Hidden (Use Show Button).")
  elseif msg=="show" then setHidden(false); print("|cFFFF77FFGlitterHealth|r: Bars Shown.")
  
  elseif msg=="debugjumps" then
    DB.profile.debugJumps = not DB.profile.debugJumps
    print("|cFFFF77FFGlitterHealth|r: Jump debug " .. (DB.profile.debugJumps and "ON" or "OFF") .. ".")
elseif msg=="export" then openCSVWindow()
  elseif msg=="center" then if not ui.frame then buildFrame() end centerFrame(ui.frame); print("|cFFFF77FFGlitterHealth|r: Window Centered.")
  else
    print("|cFFFF77FFGlitterHealth|r Commands:")
    print("  /gh options   — Open Settings")
    print("  /gh mood      — Wellbeing Window")
    print("  /gh debugjumps — Toggle jump debug")
    print("  /gh export    — CSV Export")
    print("  /gh hide|show — Toggle Bars")
    print("  /gh lock | unlock | reset | center")
  end
  updateBars()
end